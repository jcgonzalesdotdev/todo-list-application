<template>
  <div>
    <button class="m-2" @click="modalCheck">更新</button>
    <TaskModal :modalActive="isModalActive" @close-modal="modalCheck">
      <form>
        <div class="form-input">
          <label>Title:</label>
          <input v-model="formData.title" type="text" placeholder="ex. Development Task" class="input-title" />
          <label>Description:</label>
          <textarea v-model="formData.description" placeholder="Write your description here" class="input-desc"></textarea>
          <label>Start Date:</label>
          <input v-model="formData.start_date" type="date" placeholder="Start Date" class="input-startD" />
          <label>End Date:</label>
          <input v-model="formData.end_date" type="date" placeholder="End Date" class="input-endD" />
        </div>
        <!-- status checkbox -->
        ...
        <div class="form-button">
          <button class="button-save" type="submit" @click="updateTask">Update</button>
...
export default {
  ...
  methods: {
    ...
    async updateTask() {
      await TaskService.updateTask(this.formData.id, this.formData).then((response) => {
